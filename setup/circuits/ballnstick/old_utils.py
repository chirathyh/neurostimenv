...
def setup_network_ballnstick(network, args, MPI_VAR):
    ...
    w = args.env.network.recurrent_weights
    inhibition_scale = args.env.network.inhibition_scale

    weightArguments = [
        [
            dict(loc=w.ee_mean, scale=0.1 * w.ee_mean),
            dict(loc=w.ei_mean, scale=0.1 * w.ei_mean),
        ],
        [
            dict(
                loc=w.ie_mean * inhibition_scale,
                scale=0.1 * w.ie_mean * inhibition_scale,
            ),
            dict(
                loc=w.ii_mean,
                scale=0.1 * w.ii_mean,
