...
* Go to the project folder (neurostimenv): <code>cd neurostimenv</code>
...
* The neural environment is based on the <code>NEURON==8.2.3</code> and <code>LFPy==2.3</code>.
* The project uses the <code>SimNIBS==4.1.0</code> for the transcranial stimulation.
...
Running a Implicit Q-Learning algorithm.
```
cd experiments/drl 
...
* Running large neural circuits require a high-performance computing environment (e.g., <code>env=hl23net</code> with N=1,000 neurons).
* To test the framework on your local machine, use the <code>env=ballnstick</code>, which uses a simple circuit (N=40) without any biophesical mechanisms.
