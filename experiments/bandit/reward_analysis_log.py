...
# log normal outlier
x = np.asarray(reward_values_final_segment) * -1
log_x = np.log(x)


# box cox transofrms
x_positive = x[x > 0]
x_boxcox, lambda_bc = stats.boxcox(x_positive)
print(f"Box-Cox lambda: {lambda_bc:.4f}")
mean_bc = np.mean(x_boxcox)
std_bc = np.std(x_boxcox)
lower_bound = mean_bc - 2.5 * std_bc
upper_bound = mean_bc + 2.5 * std_bc
mask = (x_boxcox >= lower_bound) & (x_boxcox <= upper_bound)
x_boxcox_filtered = x_boxcox[mask]
x_filtered = x_positive[mask]  # back to original scale (untransformed values)
print("Filtered values:", x_filtered)
print("Original sample size:", len(x_positive))
print("Filtered sample size:", len(x_filtered))

alpha =0.05
stat, p = shapiro(log_x)
label="_log_x"
print(f"  Shapiro-Wilk {label}: W={stat:.4f}, p={p:.4f} ({'reject normality' if p < alpha else 'fail to reject normality'})")
stat, p = shapiro(x_boxcox)
label="_x_boxcox"
print(f"  Shapiro-Wilk {label}: W={stat:.4f}, p={p:.4f} ({'reject normality' if p < alpha else 'fail to reject normality'})")
exit()
...
plt.figure()
plt.hist(x, bins=20, alpha=0.5, label='original')
plt.hist(log_x, bins=20, alpha=0.5, label='log transform')
plt.hist(x_boxcox, bins=20, alpha=0.5, label='box cox transform')
plt.legend()
plt.show()
exit()

q1 = np.percentile(log_x, 25)
q3 = np.percentile(log_x, 75)
iqr = q3 - q1
# Step 3: Compute fences in log-space
lower_fence_log = q1 - 1.5 * iqr
upper_fence_log = q3 + 1.5 * iqr
# Step 4: Convert fences back to original scale
lower_fence = np.exp(lower_fence_log)
upper_fence = np.exp(upper_fence_log)

# Step 5: Identify non-outliers
...
std_log = np.std(log_x)
lower_fence_log = mean_log - 1.5 * std_log
upper_fence_log = mean_log + 1.5 * std_log
lower_fence = np.exp(lower_fence_log)
upper_fence = np.exp(upper_fence_log)
mask = (x >= lower_fence) & (x <= upper_fence)
x_filtered = x[mask]
print(x_filtered)
print(f"Filtered sample size: {len(x_filtered)}")
exit()
