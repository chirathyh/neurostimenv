...
from agent.ucbbandit import RobustUCBBandit
...
def main(cfg: DictConfig) -> None:
    ...
    if RANK==0:
        bandit = RobustUCBBandit(cfg.agent.n_arms, pretrain=cfg.agent.pretrain, checkpoint=cfg.agent.checkpoint)
    else:
        bandit = None
    COMM.Barrier()
    ...
    for t in range(cfg.agent.n_trials):
        ...
        if RANK==0:
            ...
            rewards_path = cfg.experiment.dir+"/checkpoints/rewards.pkl" # Save as .npy file
            with open(rewards_path, "wb") as f:
                pickle.dump(bandit.rewards, f)
            np.save(cfg.experiment.dir+"/checkpoints/counts.npy", bandit.counts)  # Save as .npy file
            np.save(cfg.experiment.dir+"/checkpoints/t.npy", bandit.t)  # Save as .npy file
