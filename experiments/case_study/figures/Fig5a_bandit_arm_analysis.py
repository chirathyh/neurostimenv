...
# bandit results
all_freqs_seg1, avg_psd_seg1, ci_lower_seg1, ci_upper_seg1 = process_bandit_testing(folder_path=BANDIT, selected_arm=0, segment=5)
...
all_freqs_b, avg_psd_b, ci_lower_b, ci_upper_b = process_bandit_testing(folder_path=BANDIT, selected_arm=2, segment=5)
plt.plot(all_freqs_b, avg_psd_b, color='tab:purple', label=f"Bandit Stimulation: {AMP[2]}mA, {FREQ[2]}Hz")

all_freqs_b, avg_psd_b, ci_lower_b, ci_upper_b = process_bandit_testing(folder_path=BANDIT, selected_arm=3, segment=5)
plt.plot(all_freqs_b, avg_psd_b, color='tab:brown', label=f"Bandit Stimulation: {AMP[3]}mA, {FREQ[3]}Hz")

all_freqs_b, avg_psd_b, ci_lower_b, ci_upper_b = process_bandit_testing(folder_path=BANDIT, selected_arm=4, segment=5)
plt.plot(all_freqs_b, avg_psd_b, color='tab:orange', label=f"Bandit Stimulation: {AMP[4]}mA, {FREQ[4]}Hz")

all_freqs_b, avg_psd_b, ci_lower_b, ci_upper_b = process_bandit_testing(folder_path=BANDIT, selected_arm=5, segment=5)
...
plt.grid(True)
plt.tight_layout()
fig.savefig('png/Fig5a.png',
            dpi=600,                # change to 300/400/600 depending on needs
            bbox_inches='tight',
            pad_inches=0.02,        # small padding so nothing is cut off
            facecolor='auto',       # 'white' or 'auto' (use 'transparent=True' if you want alpha)
            transparent=False)
plt.show()
