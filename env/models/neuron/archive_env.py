...
class NeuronEnv(gym.Env):
    ...
    def _step(self, action):
        ...
        # tstart = self.step_count * self.network.obs_win_len
        # tstop = tstart + self.network.obs_win_len - 1

        self.network.lfpy_state.save()
        ...
        SPIKES = self.network.step(probes=[self.extracellular.electrode], t_ext=t_ext, **self.args.env.network.networkSimulationArguments)

        # self.network.enable_extracellular_stimulation(self.extracellular.electrode, t_ext, n=5)
        # SPIKES = self.network.simulate(probes=[self.extracellular.electrode],**self.args.env.network.networkSimulationArguments)
        ...
        # # run simulation:
        # SPIKES, sim_t = self.network.step(
    ...
    def _reset(self):
        ...
        #self.network.enable_extracellular_stimulation_mpi(self.extracellular.electrode, 0, t_ext, n=5)
        self.network.init_simulation(probes=self.extracellular_models, t_ext=t_ext, obs_win_len=self.args.env.simulation.obs_win_len,
                                     **self.args.env.network.networkSimulationArguments)
        SPIKES = self.network.step(probes=[self.extracellular.electrode], t_ext=t_ext, **self.args.env.network.networkSimulationArguments)
